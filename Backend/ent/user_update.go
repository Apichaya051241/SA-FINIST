...
// AddUsers adds the users edges to Registerstore.
func (uu *UserUpdate) AddUsers(r ...*Registerstore) *UserUpdate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
...
// RemoveUsers removes users edges to Registerstore.
func (uu *UserUpdate) RemoveUsers(r ...*Registerstore) *UserUpdate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
...
func (uu *UserUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := uu.mutation.RemovedUsersIDs(); len(nodes) > 0 {
...
// AddUsers adds the users edges to Registerstore.
func (uuo *UserUpdateOne) AddUsers(r ...*Registerstore) *UserUpdateOne {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
...
// RemoveUsers removes users edges to Registerstore.
func (uuo *UserUpdateOne) RemoveUsers(r ...*Registerstore) *UserUpdateOne {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
...
func (uuo *UserUpdateOne) sqlSave(ctx context.Context) (u *User, err error) {
	...
	if nodes := uuo.mutation.RemovedUsersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.UsersTable,
