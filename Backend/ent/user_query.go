...
func (uq *UserQuery) QueryUsers() *RegisterstoreQuery {
	query := &RegisterstoreQuery{config: uq.config}
...
func (uq *UserQuery) WithUsers(opts ...func(*RegisterstoreQuery)) *UserQuery {
	query := &RegisterstoreQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUsers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_users
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_users" returned %v for node %v`, *fk, n.ID)
			}
